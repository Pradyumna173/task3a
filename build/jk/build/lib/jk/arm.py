...
class Arm(Node):
    ...
    def pick_box(self):
        ...
        if len(self.box_dict) > 1:
            box_0 = list(self.box_dict)[0]
            box_1 = list(self.box_dict)[1]

            box_pose_0 = self.box_dict[box_0]
            box_pose_1 = self.box_dict[box_1]

            if box_pose_0[1] > box_pose_1[1]:
                self.box_number = list(self.box_dict)[0]
                box_pose = box_pose_0
            else:
                self.box_number = list(self.box_dict)[1]
                box_pose = box_pose_1
        else:
            self.box_number = list(self.box_dict)[0]
            box_pose = self.box_number
        ...
        if state == 0:
            ...
            if EBOT_ID == 6:
                ...
                if self.force > 65.0:  # only for hardware
                    error_z = 0.0
                    self.state = 3
            else:
    ...
    def passingServerCB(self, request, response):
        ...
        self.servo_lookup()
        error_z = (self.saved_z - 0.20) - self.servo_z
        ...
        self.get_logger().info("Passing Service returned")
