...
def detect_aruco(image):
    ...
    if ids is not None:
        for corner, marker_id in zip(corners, ids):
            ...
            print(
                f"Marker ID: {marker_id[0]}, Translation (x, y, z): ({tvec[0][0][0]:.2f}, {tvec[0][0][1]:.2f}, {tvec[0][0][2]:.2f})"
            )
...
class ArucoTF(Node):
    ...
    def pick_box(self):
        ...
        while (abs(error_x) > 0.02) or (abs(error_y) > 0.02) or (self.force < 90.0):
            error_x = goal_x - self.servo_x
            error_y = goal_y - self.servo_y
    ...
    def pass_box(self, box_number):
        ...
        error_x = goal_x - self.servo_x
        error_y = goal_y - self.servo_y
        error_z = (self.temp_z + 0.1) - self.servo_z
        # error_z = 0.0
